              ...
              {options.map((option) => {
                ...
                return (
                  ...
                  >
                    ...
                    >
                      <Checkbox className={"bg-white"} checked={isSelected} />
                    </div>
                    {/* {option.icon && (
...
export function NumberFilter<T>({
  ...
  const facets = column.getFacetedUniqueValues();
  const selectedValues = new Set(column.getFilterValue() as NumberValue[]);

  const options = Array.from(facets.keys())
    .map((value) => ({
      value: value ? (value as NumberValue) : null,
      label: value ? (value as number) : "No value",
    }))
    .sort((a, b) => {
