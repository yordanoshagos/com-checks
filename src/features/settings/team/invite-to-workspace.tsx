...
export default function InviteToOrganization() {
  ...
  const mutation = api.organization.inviteUsers.useMutation({
    onSuccess: async ({ count }) => {
      ...
      if (count > 0) {
        toast.success(
          `Successfully invited ${form.getValues("emails").length} user${form.getValues("emails").length > 1 ? "s" : ""}`,
        );
      }
      setOpen(false);
    },
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button variant="outline">Add user to organization</Button>
      </DialogTrigger>
              ...
              render={() => (
                <FormItem className="flex flex-col gap-y-2">
                  ...
                  <FormDescription className="text-sm text-gray-500">
                    ...
                    after each one. They will have access to your organization's
                    Complēre studies.
                  </FormDescription>
                  <FormMessage />
                </FormItem>
